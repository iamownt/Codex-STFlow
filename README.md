...
HEST can now be extended by extracting embeddings and running STFlow for the remaining Table 1 tasks. The highest-priority next tasks are `IDC`, `LYMPH`, `PRAD`, and `CCRCC` because they are prominent in the paper and exercise different slide counts.
