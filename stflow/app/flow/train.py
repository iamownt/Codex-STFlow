...
def resolve_device(device_arg):
    ...
    if device_arg.startswith("cuda"):
        torch.cuda.set_device(torch.device(device_arg))
    return device_arg


def make_eval_loaders(args, sample_ids, normalize_method):
    sample_id_paths = [
        HESTDatasetPath(
            name=sample_id,
...
def main(args, split_id, train_sample_ids, eval_sample_ids, val_save_dir, checkpoint_save_dir, test_sample_ids=None):
    ...
    val_loaders = make_eval_loaders(args, eval_sample_ids, normalize_method)
    test_loaders = make_eval_loaders(args, test_sample_ids, normalize_method) if test_sample_ids is not None else None
    ...
    diffusier = Interpolant(
        args.prior_sampler, 
        total_count=args.zinb_total_count,
        logits=args.zinb_logits,
    ...
    print("Training")
    best_pearson, best_val_dict, best_result_dict = -1, None, None
    early_stop_step = 0
        ...
        if epoch % args.eval_step == 0 or epoch == args.epochs:
            ...
            if val_perf_dict["all"]['pearson_mean'] > best_pearson:
                ...
                if test_loaders is not None:
                    test_perf_dict, _ = test(args, diffusier, model, test_loaders, return_all=True)
                    for patch_name, dataset_res in test_perf_dict.items():
                        with open(os.path.join(val_save_dir, f'test_{patch_name}_results.json'), 'w') as f:
                            json.dump(dataset_res, f, sort_keys=True, indent=4)
                    best_result_dict = test_perf_dict["all"]
                else:
                    best_result_dict = best_val_dict["all"]
...
def run(args):
    ...
    split_ids = sorted(
        int(name.removeprefix("train_").removesuffix(".csv"))
        for name in splits
        if name.startswith("train_") and name.endswith(".csv")
    )
    all_split_results = []
    ...
    for i in split_ids:
        ...
        val_path = os.path.join(split_dir, f'val_{i}.csv')
        ...
        if val_df is not None:
            eval_sample_ids = val_df['sample_id'].tolist()
            final_sample_ids = test_sample_ids
        else:
            eval_sample_ids = test_sample_ids
        ...
        results = main(args, i, train_sample_ids, eval_sample_ids, kfold_save_dir, checkpoint_save_dir, final_sample_ids)
